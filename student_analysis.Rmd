...
```{r}
m1<-tidy(fit1) %>%
  filter(term == "SI.Attended" | term == "URMURM" | term == "First.Generation.FlagY" | term == "HS.GPA" | term == "Inst.MD.Persn.Onl.OthrOnline")
dwplot(m1) +
...
```{r}
m3<-tidy(fit2) %>%
  filter(term == "SI.Visit.Num" | term == "URMURM" | term == "First.Generation.FlagY" | term == "HS.GPA" | term == "Inst.MD.Persn.Onl.OthrOnline")
dwplot(m3) +
    geom_vline(xintercept = 0, colour = "grey60", linetype = 2)
```
...
```{r}
dod_mod <- glm(One.Year.Retention ~ SI.Attended*URM,  family = binomial(link = "logit"), data=dod_data)
coef(dod_mod)
