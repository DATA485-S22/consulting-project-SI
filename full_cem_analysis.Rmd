...
matched_df2_wide <- matched_df2 %>% 
  pivot_wider(id_cols = c(Random.Course.ID, SI.Attended, Term.Year), 
...
ggplot(matched_df2_wide, 
  ...
  geom_text(aes(x=.75, y=.25, label = "Non-FG > FG"), col="red") +
  geom_text(aes(x=.75, y=-.25, label = "Non-FG < FG"), col="green3") +
  ylab("Non-FG - FG") + theme(legend.position = "none") +
  ggtitle("FG gap among matched students who went to SI vs those who did not go to SI") + 
...
matched_df1 %>% ggline(x="First.Generation.Flag", y = "grade", 
  ...
  ggtitle("SI helps both groups equally")
...
The estimated effect was (0.344, SE=.076, p<.0001), indicating that the average effect of attending SI 0.338 grade points higher compared to those that did not go to SI. 
...
```{r}
dod_data <- data %>% 
  mutate(dwf = ifelse(Student.Class.Grade.Point.per.Unit < 2.0, 1, 0))
```

Bivariate exploration. No additional covarites.
```{r}
dod_data %>% group_by(IPEDS.Ethnicity.URM.Non.URM, SI.Attended) %>%
  summarize(x = sum(dwf), 
            n=n(), 
            pct = x/n, pct.lab = percent(pct, accuracy = .1))  %>%
