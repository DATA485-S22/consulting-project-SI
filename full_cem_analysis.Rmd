...
matched_df2 <- matched_df1 %>% 
  ...
  group_by(Random.Course.ID, IPEDS.Ethnicity.URM.Non.URM, SI.Attended,
           Term.Year, Academic.Subject, Class.Subject.Number.Section) %>%
  summarise(avg.grade = mean(grade), 
            n=n()) 

matched_df2_wide <- matched_df2 %>% 
  pivot_wider(id_cols = c(Random.Course.ID, SI.Attended, Term.Year, Academic.Subject, Class.Subject.Number.Section), 
