...
facet_names<-c(`0` = "No SI Component", `1` = "SI Component")
...
```{r include=FALSE}
course.level$Term.Year.x<-factor(course.level$Term.Year.x)
lm1 <- lm(dwf.rate ~ URM*SI.Component.Flag + First.Gen.Perc*SI.Component.Flag + class.size + Term.Year.x + HS.GPA.Ave, data=course.level)
summary(lm1)
model1 <- tidy(lm1)
