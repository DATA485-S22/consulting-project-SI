...
model1 <- tidy(lm1) %>%
  filter(term == "URMURM" | term == "First.Gen.Perc" | term == "SI.Component.Flag")
dwplot(model1,dot_args = list(size = 3, color = "black"), whisker_args = list(size = 2, color = "grey")) + geom_vline(xintercept = 0, colour = "black",linetype = 2) + ggtitle("Model Results DWF Rate and Equity Gaps") + xlab("Confidence Interval") + theme_few() +
