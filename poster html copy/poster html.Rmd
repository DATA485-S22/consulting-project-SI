...
# Methods
### Student Level
*  The student level explores the success of an individual student. The questions we are addressing is "Does SI have an impact on the one retention" and "Does the impact of SI differ between the equity groups". 
...
```{r echo=FALSE, fig.dim=c(14,7), message=FALSE, warning=FALSE}
facet_names<-c(`0` = "Did Not Attend SI", `1` = "Attended SI")
ggplot(dod_data) +
  geom_bar(aes(x=One.Year.Retention, fill=URM),
           position = "dodge") +
  facet_wrap(~SI.Attended, labeller = as_labeller(facet_names)) + ggtitle("Figure 1") +xlab("One Year Retention")+   ylab("") + theme_few() +
  theme(text = element_text(size = 32, family = "Times New Roman"))+ scale_fill_brewer(name="Underrepresented\nMinority(URM)", palette = "Paired")

ggplot(dod_data2) +
  geom_bar(aes(x=One.Year.Retention, fill=First.Generation.Flag),
           position = "dodge") +
  facet_wrap(~SI.Attended, labeller = as_labeller(facet_names)) + ggtitle("Figure 2") +xlab("One Year Retention")+    ylab("") + theme_few() +
  theme(text = element_text(size = 32,family = "Times New Roman")) + scale_fill_brewer(name="First\nGeneration",palette = "Reds")
```
<br>
<br>
<br>
...
that did attend SI (column 1). Figure 4 compares DWF Rate of the percentage of students that 
are First Generation who did not attend SI (column 0) and students that did attend SI (column 1). Both figures show SI reduces the DWF rates of equity gaps.
...
ggplot(dod, aes(URM, dwf.rate, fill=URM)) + geom_boxplot() + facet_wrap(~SI.Component.Flag,labeller = as_labeller(facet_names)) + ylab("DWF Rate")+ scale_fill_brewer(palette = "Paired") +theme_bw() +
  theme(text = element_text(size = 32,family = "Times New Roman"))

ggplot(dod, aes(x =First.Gen.Perc, y=dwf.rate)) + geom_point(aes( color = as.factor(SI.Component.Flag))) +
  xlab("Percentage of First Generation") + ylab("DWF Rate") + scale_colour_brewer(palette = "Reds")+theme_bw() +
  theme(text = element_text(size = 32,family = "Times New Roman"))
```
...
dwplot(m1,dot_args = list(size = 3, color = "black"), whisker_args = list(size = 2, color = "grey")) +
    geom_vline(xintercept = 0, colour = "black",linetype = 2) + ggtitle("Figure 3:  SI and Equity") + xlab("Confidence Interval") + theme_few() +
  theme(legend.position = "none", axis.title.x = element_text(size = 20, family = "Times New Roman"), title = element_text(size = 32,family = "Times New Roman"), text = element_text(size = 24, family = "Times New Roman"))
m4<-tidy(fit3)%>%
...
dwplot(m4,dot_args = list(size = 3, color = "black"), whisker_args = list(size = 2, color = "grey")) +
    geom_vline(xintercept = 0, colour = "grey60", linetype = 2) + ggtitle("Figure 4: Interactions SI and Equity")+ xlab("Confidence Interval") + theme_few() +
  theme(legend.position = "none", axis.title.x = element_text(size = 20,family = "Times New Roman"), title = element_text(size = 32),text = element_text(size = 24, family = "Times New Roman"))
```
