...
d$Major.1.College<-factor(d$Major.1.College)
...
d<-select(d, c(Random.Student.ID, Random.Course.ID, Cohort.Term.Year, Degree.Term, HS.GPA, One.Year.Retention,
               ...
               GE.Class.Flag, Gender.Code, IPEDS.Ethnicity, IPEDS.Ethnicity.URM.Non.URM, URM, First.Generation.Flag,
               Major.1.STEM.Flag, Major.1.College))
write.csv(d, "data/student_analysis.csv")

fit1<-glm(One.Year.Retention ~ SLC.Attended.Flag + HS.GPA + URM + Gender.Code,
             family = binomial(link = "logit"), data = d)
summary(fit1)

fit2<-glm(One.Year.Retention ~ SLC.Attended.Flag + HS.GPA + URM + GE.Class.Flag, family = binomial(link = "logit"), data = d)
